...
- Trigger: `.premium_npc profession <key>` (e.g. `.premium_npc profession blacksmithing`) - summons that profession's trainer directly. With no key (or an unrecognized one), prints the list of valid keys instead of summoning anything - a bare dot-command has no native picker UI to show a list through. The Premium Menu item below offers an actual clickable picker instead.
- One `creature_template` entry per profession (900213-900226, `PREMIUM_NPC_CONFIG.PROFESSION_TRAINER.PROFESSIONS`), each a standalone trainer for that profession's full 0/1-to-450 skill chain (`sql/db-world/02_profession_trainer_spells.sql`) - aggregated from every real profession trainer in the game (base + Master + Grand Master + specialization trainers), deduplicated by spell, ordered low-to-high skill rank, then sliced one profession per entry. Model/`unit_class` borrowed from each profession's own real "Grand Master"-named trainer creature (e.g. Alchemy borrows Lanolis Dewdrop's model) - their own rows are never modified, only displayId is reused.
- Training is free at all 14 - a trailing `UPDATE trainer_spell SET MoneyCost = 0` at the end of `02_profession_trainer_spells.sql`, scoped to `TrainerId BETWEEN 900213 AND 900226`. Safe to scope this tightly because `MoneyCost` is looked up per-`TrainerId`/`SpellId` pair (confirmed via `Trainer.cpp`'s `TeachSpell`) - this can never discount or zero any real trainer's cost, only these 14. The real per-spell costs are kept in the `INSERT` rows above it (not zeroed inline) so they're still there for reference, or to revert by deleting the `UPDATE`.
  - An earlier draft of this redesign summoned a real, existing "Grand Master" trainer creature directly instead, for every profession where that real creature's own `trainer_spell` data already spanned its entire skill range with no gap - no new SQL needed at all, same `ENTRIES`-style pattern Class Trainer uses. Reverted: this server runs `mod-individual-progression` (confirmed enabled), and the "full range" finding for several of those real trainers couldn't be conclusively traced to either stock AzerothCore data or that module - it just didn't match Wowhead's retail reference for at least one profession (Alchemy). Building every profession from this project's own already-vetted aggregate instead avoids depending on real trainer data of uncertain provenance.
- Whichever trainer gets summoned follows the player for `PREMIUM_NPC_CONFIG.PROFESSION_TRAINER.SUMMON_DURATION_SECONDS` (120s by default), then despawns (or despawns early if a different premium NPC is summoned first). The native Trainer window shows whatever's not yet trainable as greyed/red rather than hiding it.
...
- Profession Trainer is the one exception: selecting it shows a second gossip menu (same item, same gossip session) listing all 14 professions instead of summoning anything directly - picking one of those calls `TryProfessionTrainer(player, key)`. This skips ever needing an intermediate "which profession?" NPC - the item already has its own gossip to ask from. The two menu levels are told apart by gossip `sender` (`premium_menu_item.lua`'s `TOP_SENDER`/`PROFESSION_SENDER`), since `intid` alone repeats across levels.
